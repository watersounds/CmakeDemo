...
StereoCalib(const vector<string>& imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated=true, bool showRectified=true)
{
    ...
    for( i = j = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            Mat img = imread("../image/"+filename, 0);
            if(img.empty())
                break;
            if( imageSize == Size() )
                imageSize = img.size();
...
// epipolar geometry constraint: m2^t*F*m1=0
    ...
    FileStorage fs("../bin/intrinsics.yml", FileStorage::WRITE);
    if( fs.isOpened() )
    {
    ...
    fs.open("../bin/extrinsics.yml", FileStorage::WRITE);
    if( fs.isOpened() )
    {
 ...
 // from the fundamental matrix
    ...
    for( i = 0; i < nimages; i++ )
    {
        for( k = 0; k < 2; k++ )
        {
            ...
            // imwrite("o.jpg", imread(goodImageList[i*2+k], 0));
            // imwrite("rimg.jpg", rimg);
            // imwrite("cimg.jpg", cimg);
            printf("302 imread K %d\n", k);
            remap(img, rimg, rmap[k][0], rmap[k][1], INTER_LINEAR);
            printf("304 remap K %d\n", k);
